				...
				else
				{
					...
					if ($order_carrier->update())
					{
						...
						if ($customer->email) {
							...
							if (@SMS::Send((int)$order->id_lang, 'in_transit', SMS::l('Package in transit', (int)$order->id_lang), $templateVars,
								$customer->phone, $customer->firstname.' '.$customer->lastname, (int)$order->id_shop))
							{
								Hook::exec('actionAdminOrdersTrackingNumberUpdate', array('order' => $order, 'customer' => $customer, 'carrier' => $carrier), null, false, true, false, $order->id_shop);
								Tools::redirectAdmin(self::$currentIndex.'&id_order='.$order->id.'&vieworder&conf=4&token='.$this->token);
							}
							else
								$this->errors[] = Tools::displayError('An error occurred while sending an phone to the customer.');
						}
					}
					else
